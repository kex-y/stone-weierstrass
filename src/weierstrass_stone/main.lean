...
lemma closure₀_closed_with_sup {M₀ : set (bounded_continuous_function X ℝ)}
{f g} (hf : f ∈ closure₀ M₀) (hg : g ∈ closure₀ M₀) : f ⊔ g ∈ closure₀ M₀ := sorry

lemma closure₀_closed_with_inf {M₀ : set (bounded_continuous_function X ℝ)}
{f g} (hf : f ∈ closure₀ M₀) (hg : g ∈ closure₀ M₀) : f ⊓ g ∈ closure₀ M₀ := sorry
...
- If ∀ x, y ∈ X, a, b ∈ ℝ, ∃ f ∈ M₀, f(x) = a, f(y) = b, then 
  closure₂ M₀ = M (= @univ bounded_continuous_function X ℝ)
where M is the set of all bounded continuous functions from X to ℝ.

- If f : ℕ → bounded_continuous_function X ℝ is monotone and converges 
...
def boundary_points (M : set (bounded_continuous_function X ℝ)) (x y : X) := 
  {z | ∃ (f : bounded_continuous_function X ℝ) (hf : f ∈ closure₀ M), (f x, f y) = z}
...
lemma in_closure'_of_in_closoure₂ {f : bounded_continuous_function X ℝ} 
(h : f ∈ closure₂ M₀) : ∀ x y : X, (f x, f y) ∈ closure' (boundary_points M₀ x y) :=
begin
  intros x y, right,
...
lemma bcf_pair_sup_eq_bcf_sup_pair {u v : bounded_continuous_function X ℝ} {x y} : 
(u x, u y) ⊔ (v x, v y) = ((u ⊔ v) x, (u ⊔ v) y) := rfl

lemma bcf_pair_inf_eq_bcf_inf_pair {u v : bounded_continuous_function X ℝ} {x y} : 
(u x, u y) ⊓ (v x, v y) = ((u ⊓ v) x, (u ⊓ v) y) := rfl

lemma in_closure₂_of_in_closure' {f : bounded_continuous_function X ℝ} 
...
lemma in_closure₂_iff_in_closure' {f : bounded_continuous_function X ℝ} : 
...
then M₀ = M₁.
-/
lemma boundary_points_eq_of_eq (M₀ M₁ : set (bounded_continuous_function X ℝ))
(h : M₀ = M₁) : ∀ x y : X, boundary_points M₀ x y = boundary_points M₁ x y := 
λ x y, by rw h

lemma eq_of_boundary_points_eq (M₀ M₁ : set (bounded_continuous_function X ℝ))
...
theorem eq_iff_boundary_points_eq (M₀ M₁ : set (bounded_continuous_function X ℝ))
